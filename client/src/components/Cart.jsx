...
const Cart = forwardRef(({ tableId, onOrderComplete }, ref) => {
    ...
    const fetchTaxes = async () => {
        try {
            setLoading(true);
            const response = await axios.get('http://15.207.252.191:500/api/taxes');
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://15.207.252.191:5000/api/orders', orderData);
            console.log('Cart: Order created successfully:', response.data);

            // Generate bill
            const billResponse = await axios.get(`http://15.207.252.191:5000/api/orders/${response.data.id}/bill`);
    ...
    const handleCloseBill = async () => {
        try {
            ...
            await axios.patch(`http://15.207.252.191:5000/api/tables/${tableId}/status`, {
                status: 'available'
            });
